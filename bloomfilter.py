...
"""
    布隆过滤器集成到 scrapy 中
    1、将 redis 连接 和 PyBloomFilter 导入到 scrapy_redis/dupefilter.py 中；
    2、在 RFPDupeFilter 类的构造函数中实例化 PyBloomFilter， self.bf = PyBloomFilter(conn=conn, key=key)
    3、重写 request_seen，在 fp = self.request_fingerprint(request)之后添加逻辑：
        if self.bf.is_exist():
            return True
        else:
            self.bf.add(fp)
